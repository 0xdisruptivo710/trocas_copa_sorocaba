-- Adiciona lat_approx / lng_approx ao retorno de trocas_find_matches.
-- Coordenadas são "snapped" a uma grade de 0.005° (~500m em lat,
-- ~510m em lng na latitude de Sorocaba) — esconde a casa mas permite
-- plotar o pin no mapa.
--
-- Mantém todas as colunas anteriores; UI antiga ignora os campos novos.
...
begin
  ...
  select s.id, s.username, s.full_name, s.avatar_url, s.city, s.state, s.distance_km,
    ...
    -- Snap a ~500m: protege endereço exato. NULL quando user não tem localização.
    case when s.location is not null
      then st_y(st_snaptogrid(s.location::geometry, 0.005))
      else null end as lat_approx,
    case when s.location is not null
      then st_x(st_snaptogrid(s.location::geometry, 0.005))
